...
- OCR lives in `apps/api/app/ocr.py` and is exposed via `/api/projects/{project_id}/documents/{document_id}/ocr` plus `/api/ocr`; it validates project/document consistency with SQL text queries before OCR, rate-limits per client in memory, selects Mock/Azure/Gemini through `get_ocr_provider`, falls back to mock when Azure/Gemini credentials are absent only outside `APP_ENV=staging|production`, returns HTTP 503 for missing OCR credentials in staging/production, and must never log OCR provider keys.
